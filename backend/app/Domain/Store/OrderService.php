...
class OrderService
{
    ...
    public function place(User $user, array $lines, string $idempotencyKey, ?string $addressId = null, string $paymentMode = 'points', ?string $note = null): array
    {
        ...
        try {
            $order = DB::transaction(fn () => $this->placeLocked($user, $lines, $idempotencyKey, $addressId, $note), attempts: 3);
    ...
    private function placeLocked(User $user, Collection $lines, string $key, ?string $addressId, ?string $note): Order
    {

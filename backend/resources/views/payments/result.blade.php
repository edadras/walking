...
<div class="card">
  @if ($paid)
    ...
    <p>{{ $label ?? 'سفارش '.$number }} — {{ number_format($amount) }} ریال</p>
  ...
  @elseif ($pending)
    ...
    <p>ارتباط با درگاه برقرار نشد. نتیجه تا چند دقیقه دیگر {{ isset($label) ? 'در پنل' : 'در بخش سفارش‌ها' }} نمایش داده می‌شود؛ اگر مبلغی کسر شده باشد از بین نمی‌رود.</p>
  @else
    <div class="icon" style="background:var(--danger)">✕</div>
    <h1>پرداخت انجام نشد</h1>
    <p>{{ $label ?? 'سفارش '.$number }} {{ isset($label) ? 'انجام نشد' : 'لغو شد' }} و مبلغی کسر نشده است. اگر مبلغی کسر شده، طبق قوانین بانکی حداکثر تا ۷۲ ساعت به حسابت برمی‌گردد.</p>
  @endif
  <a class="btn" href="{{ $appLink }}">{{ $backLabel ?? 'بازگشت به گام‌یار' }}</a>
</div>
</body>
</html>
